...
public class CameraPositionMapActivity extends FragmentActivity implements
	...
	private void markAll() {
		...
		if (mMap != null) {
			...
			// when load complete mark our position
			  ...
			  myMarker = mMap.addMarker(new MarkerOptions() .position(new
			  LatLng(Info.lat, Info.lng))
			  .title(getString(R.string.you_here_msg))
			  .snippet(Info.reverseGpsName) .icon(BitmapDescriptorFactory
			...
			for (int i = 0; i < this.camListFilter.size(); i++) {
				myMarker(this.camListFilter.get(i).lat,
						this.camListFilter.get(i).lng,
						this.camListFilter.get(i).thaiName,
						this.camListFilter.get(i).id);
			}
	...
	private void myMarker(String sLat, String sLng, String title, String id) {
		...
		double dLat = Double.parseDouble(sLat + "0");
		double dLng = Double.parseDouble(sLng + "0");
		accidentLatLng = new LatLng(dLat, dLng);
